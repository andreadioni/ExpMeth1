...
```{r}
genderbalancing <-  pd %>% group_by(gender) %>% summarize(mean(balloon_balance))
genderbalancing
```
Males are generally better at balloon balancing than women

2. Is there a relation between sound level preference and which cola was chosen?

```{r}
colasound <- pd %>% group_by(pd$taste_cola) %>% summarize(mean(sound_level_pref), n())
colasound #$ tegn fucks everything up
```
...
```{r}
tonguehands <- pd %>% group_by(pd$handedness) %>% summarize(mean(tongue_twist))
tonguehands
tonguehands1 <- mutate(pd, n() ) #STILL NOT WORKING
```
...
```{r}
ggplot(pd) # the most basic setup
```
...
ggplot(pd, aes(x=gender)) # if only X-axis is known. The Y-axis can be specified later
...
```{r}
ggplot(pd, aes(x=tongue_twist))+
  geom_histogram(binwidth = 3) #you can change binwidth if you want to, it's cosmetic
...
ggplot(pd, aes(x=choose_rand_num))+ 
...
```{r}
ggplot(pd, aes(x=gender, y=balloon))+
...
ggplot(pd, aes(x=gender, y=balloon, fill = gender))+
...
p <- ggplot(pd, aes(x=handedness, y=shoesize, fill = handedness))+
  geom_bar(stat='summary', fun.y = mean, width = 0.5)+
  geom_errorbar(stat='summary', fun.data = mean_se, width = 0.2)+
  labs(x="handedness", y = "shoesize")+
  scale_fill_brewer(palette="Spectral") +  #too get colors
  coord_cartesian(ylim = c(35,45))+
  theme_grey()
p

warnings()
 colors() # shows me all available colors
```
